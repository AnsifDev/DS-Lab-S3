...
void main() {
	...
	struct Iterator *iter2 = new_Iterator(list2);			//For Debugging
	...
	printf("List2: ");										//For Debugging
	resetIter(iter2);										//For Debugging
	while(hasNextNode(iter2)) {								//For Debugging
		int value = nextNode(iter2)->value;					//For Debugging
		printf("\t%d", value);								//For Debugging
	}														//For Debugging
	printf("\n");											//For Debugging
